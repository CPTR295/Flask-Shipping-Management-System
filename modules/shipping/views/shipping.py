from flask import render_template,request,flash,current_app,abort
...
from modules.shipping.services.devivery import get_all_did
...
def add_delivery_officer():
    if request.method == 'POST':
        current_app.logger.info('Add_delivery_officer post view')
        ...
        officer = DeliveryOfficer(id=int(request.form['id']),firstname=request.form['firstname'],middlename=request.form['middlename'],
                                  lastname=request.form['lastname'],mobile=request.form['mobile'],address=request.form['address'],
                                  email=request.form['email'],status=request.form['status'])
        res = repo.insert(officer=officer)
        if res == False:
            abort(500) 
        return render_template('add_delivery_officer_form.html'),200
    current_app.logger.info('add_delivery_officer get view')
    return render_template('add_delivery_officer_form.html'),200
